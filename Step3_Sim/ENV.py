...
from Step0_DNN import BLSTM_ATT
...
class ENV:
    ...
    def __init__(self):
        self.data = pd.read_csv('../Step0_DNN/DATA/rdata.csv').iloc[:,1:]
        self.action_table = np.load('../Step1_offlineDRL/DATA/action_table1.npy',allow_pickle=True).tolist()
        self.index = np.load('../Step1_offlineDRL/DATA/index.npy',allow_pickle=True).tolist()
        ...
        param=np.load('../Step0_DNN/model/BLSTM_ATT_param.npy',allow_pickle=True).tolist()
        self.net = BLSTM_ATT.Net(param)
        self.net = torch.load('../Step0_DNN/model/BLSTM_ATT')
    ...
    def reset(self,timepoint,step):
        ...
        self.results['action'] = [self.data.values[timepoint+24,self.title['control']].tolist()]
